...
const GovtScrappedJobs = () => {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      ...
      try {
        const response = await axios.get(`https://web-scraps.onrender.com/ncs/?page=${currentPage}`);
  ...
  const filterJobs = (query) => {
    const filtered = jobs.filter(job =>
      ...
      job.job_location?.toLowerCase().includes(query.toLowerCase())
    );
  ...
  return (
    <div className={`min-h-screen w-full overflow-y-auto ${isDarkMode ? 'bg-gray-900 text-gray-300' : 'bg-gray-100 text-gray-900'}`}>
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-6">Govt Jobs</h1>
        <input
          type="text"
...
export default GovtScrappedJobs;
